...
describe("Testes adicionais para validações e bordas de comparação", () => {
  ...
  test("isImpar deve retornar falso para número par negativo", () => {
    expect(isImpar(-4)).toBe(false);
  });

  test("isDivisivel deve retornar falso quando não for divisível", () => {
    expect(isDivisivel(7, 3)).toBe(false);
  });

  test("isMaiorQue deve retornar falso quando números forem iguais", () => {
    expect(isMaiorQue(5, 5)).toBe(false);
  });

  test("isMenorQue deve retornar falso quando números forem iguais", () => {
    expect(isMenorQue(5, 5)).toBe(false);
  });

  test("isEqual deve retornar falso quando números diferentes", () => {
  ...
  test("clamp deve retornar min corretamente quando valor = min", () => {
